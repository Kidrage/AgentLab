...
**当前版本完整能力手册：**[中文](CURRENT_VERSION_CAPABILITIES.zh-CN.md) · [English](CURRENT_VERSION_CAPABILITIES.en-US.md)
...
- CLI 命令面：253 个顶层命令
- 根级项目交接：`./agentlab.sh repository-handoff --repo <path> --write` 只刷新唯一权威 `PROJECT_HANDOFF.md`；共享镜像需显式 `--shared-copy`
...
- 本地 terminal chat 与旧 Codex handoff/resume 读取兼容；新任务使用受限 AgentLab role session。
...
- 本地 dashboard、任务详情、JSON API 和 SSE task events。
- 写 API 需要 `AGENTLAB_WEB_UI_TOKEN`；默认仅绑定 `127.0.0.1`。
