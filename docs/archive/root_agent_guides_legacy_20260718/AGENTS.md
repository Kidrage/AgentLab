...
- Before reading project content, run `./agentlab.sh repository-handoff --repo <path>`;
  ...
  paths/metadata without bulk-reading contents and writes the single canonical
  `PROJECT_HANDOFF.md`. Older `.agentlab/HandOff.md`, `agent_docs/HandOff.md`, and
  root `HandOff.md` variants are read-only compatibility sources.
- Refresh canonical `PROJECT_HANDOFF.md` after every material repository/project
  change and before final reporting. Use `--shared-copy` only for an explicit
  cross-endpoint mirror; a read-only repository falls back to shared memory.
