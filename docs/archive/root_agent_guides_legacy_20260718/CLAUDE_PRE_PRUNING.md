...
Active role prompts are registered by `config/agent_registry.yml`. A CLI shell
receives only its assigned role packet; retired full-driver prompts are kept
under `docs/archive/codex_full_driver_legacy_20260718/` for history only.
