...
角色/能力选择阶段产生的授权会标记 `authorizes_execution: false`，只证明该选择可以
继续规划；它不能直接授权 shell、写文件或外部调用。运行时用具体命令、路径、provider
与成本复核后产生的授权才可标记 `authorizes_execution: true`。
...
外部执行器还必须先通过 `config/executor_router.yml#safety`。网络执行、远程 clone、
外部脚本、secret、订阅凭证和未审查 merge 的禁止项都会在自动批准前执行；已启用的
禁止项返回 `forbidden`，不会被自动授权覆盖。非私密、范围明确且成本已知的普通网络
调用可在该 safety 配置允许时自动授权。
