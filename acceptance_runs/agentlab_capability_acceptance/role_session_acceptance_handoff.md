...
This is not a default AgentLab production workflow. It is a minimal trusted-runner acceptance run that loads Crown_of_Ash project context through the configured AgentLab role-session worker and returns run-local candidate artifacts for structural QC.

## Risk That Requires Explicit Approval

The selected role-session acceptance commands load Crown_of_Ash project context and may send it to configured role-session providers:
