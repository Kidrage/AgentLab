import re
from .api import json_response, build_error
...
def handle_project_detail(path):
    ...
    if not sub_route:
        return json_response({"page": "project_detail", "project_id": project_id, "known": state.known})
    elif sub_route in ["timeline", "costs", "phases", "artifacts", "routes", "approvals"]:
        return json_response({
            "page": f"project_{sub_route}",
            "project_id": project_id,
            "cost": state.cost_summary if sub_route == "costs" else None
        })
    else:
        return build_error("not found", 404)
