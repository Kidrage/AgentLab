...
_MALE_NAIL_MATERIAL_LINK = (
    r"(?:(?:外覆|覆盖着?|覆有|各有|长有|包着|包覆|覆着)"
    r"(?:一层|一片|一枚|一块)?(?:的)?)?"
)
_MALE_NAIL_CIRCUMLOCUTION_PATTERNS = (
    re.compile(
        ...
        + _MALE_NAIL_MATERIAL_LINK
        + r"(?:透明|半透明)?(?:的)?"
        r"(?:角层|角质|硬壳|硬层|硬片|硬质|薄层|覆盖层|甲片)"
        r".{0,24}(?:边缘|表面|削|剪|修|磨|齐|圆|方|尖|色泽|光泽|颜色|涂层|上色|着色|抛光)"
    ),
    re.compile(
        r"(?:(?:十枚|十个)(?:指端|趾端|末梢|末端)|"
        r"(?:双手|双足|手部|足部)(?:末梢|末端)(?:的)?(?:十枚|十个))"
        r"(?:的)?"
        + _MALE_NAIL_MATERIAL_LINK
        + r"(?:透明|半透明)?(?:的)?"
        r"(?:角层|角质|硬壳|硬层|硬片|硬质|薄层|覆盖层|甲片)"
        r".{0,24}(?:削|剪|修|磨|齐|圆|方|尖|色泽|光泽|颜色|涂层|上色|着色|抛光)"
    ),
)
