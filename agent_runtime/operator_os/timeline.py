...
def build_timeline(project_root: Path, read_errors: list[dict[str, str]] | None = None) -> list[dict[str, Any]]:
    ...
    ah = _load_yaml(brain_dir / "acceptance_history.yml", {}, project_root, read_errors)
    acceptance_source = _rel(brain_dir / "acceptance_history.yml", project_root)
    ...
    fs = _load_yaml(brain_dir / "project_fact_snapshot.yml", {}, project_root, read_errors)
        ...
        for event in fs["events"]:
            if isinstance(event, dict):
                ...
                timeline.append(_event("state_transition_applied", ts, {
                    ...
                    "source": _rel(brain_dir / "project_fact_snapshot.yml", project_root),
                }))

    # ── operator action audit events ───────────────────────────────────
    oal = _load_yaml(brain_dir / "operator_action_ledger.yml", {}, project_root, read_errors)
    ...
    for entry in action_entries:
        ...
        timeline.append(_event("operator_action_recorded", str(entry.get("recorded_at") or ""), {
            ...
            "source": _rel(brain_dir / "operator_action_ledger.yml", project_root),
        }))
    ...
    if runs_dir.exists():
        for task_dir in sorted(runs_dir.iterdir()):
            ...
            if er_path.exists():
                er = _load_yaml(er_path, {}, project_root, read_errors)
                ...
                if isinstance(env, dict):
                    ...
                    timeline.append(_event("executor_result_received",
                        str(env.get("finished_at") or env.get("recorded_at") or ""), {
                            ...
                            "status": status,
                            "source": _rel(er_path, project_root),
            ...
            if dc_dir.is_dir():
                for card_path in sorted(dc_dir.glob("*.yml")):
                    card = _load_yaml(card_path, {}, project_root, read_errors)
                    if isinstance(card, dict):
                        ...
                        timeline.append(_event(etype,
                            str(card.get("resolved_at") or card.get("created_at") or ""), {
                                ...
                                "status": s,
                                "source": _rel(card_path, project_root),
                            }))
            ...
            if cg_dir.is_dir() and any(cg_dir.iterdir()):
                for cg_path in sorted(cg_dir.glob("*.yml")):
                    timeline.append(_event("capability_gap_raised", "", {
                        "task_id": task_id,
                        "source": _rel(cg_path, project_root),
                    }))

            # artifact_promoted / artifact_archived
            pa_path = task_dir / "phase_acceptance.yml"
            if pa_path.exists():
                pa = _load_yaml(pa_path, {}, project_root, read_errors)
                    ...
                    if isinstance(st, dict):
                        ...
                        if st.get("applied"):
                            timeline.append(_event("artifact_promoted", ts, {
                                "task_id": task_id,
                                "source": _rel(pa_path, project_root),
                            }))
...
def build_failure_narrative(project_root: Path, phase_id: str | None = None) -> dict[str, Any]:
    ...
    next_actions = _load_yaml(brain_dir / "next_actions.yml", {}, project_root, None)
    ...
    return {
        ...
        "source_documents": {
            "acceptance_history": _rel(brain_dir / "acceptance_history.yml", project_root),
            "next_actions": _rel(brain_dir / "next_actions.yml", project_root),
        },
    }
...
def _load_yaml(
    ...
    try:
        ...
        if read_errors is not None:
            read_errors.append({
                "path": _rel(path, project_root or path.parent),
