...
def write_project_memory(
    ...
    append_markdown(
        ...
        "# Agent Changelog\n\n",
        f"## {utc_now()} - {task_id}\n\n"
        "- Created/updated Coding workspace memory from local deterministic scan.\n"
        f"- Command: `{command}`\n\n",
    )
...
def write_task_run(
    ...
    workflow_plan = {
        ...
        "route": {
            ...
            "agents": route,
            "rationale": [
                "Full workspace memory build requested.",
                "Route recorded for audit, but execution used local deterministic scanner only.",
            ],
            "skipped_agents": ["CodexPromptGenerator"],
    ...
    write_text(run_dir / "03_research_notes.md", render_research_notes())
    write_text(run_dir / "04_interface_map.md", render_interface_registry(projects))
    write_text(run_dir / "06_implementation_report.md", render_implementation_report(command))
    write_text(run_dir / "07_validation_report.md", render_validation_report(workspace))
    write_text(run_dir / "08_audit_report.md", render_audit_report(target))
    write_text(run_dir / "verification_report.md", render_verification_report(workspace))
    write_text(run_dir / "09_archive_update.md", render_archive_update(project, task_id, target))
    ...
    write_yaml(run_dir / "state.yml", {
        ...
        "reports": {
            ...
            "Coder": str(run_dir / "06_implementation_report.md"),
    ...
    for node_id in LIFECYCLE_NODES:
        ...
        lifecycle["nodes"][node_id]["completed_at"] = utc_now()
        lifecycle["nodes"][node_id]["skip_reason"] = None
...
def render_implementation_report(command: str) -> str:
    return (
        "# Implementation Report\n\n"
        "## Backend\n"
        "AgentLab local deterministic workspace scanner.\n\n"
        "## Files Changed\n"
        "- AgentLab project memory under `projects/CodingWorkspace/agent_docs/`\n"
        "- AgentLab task run artifacts under `projects/CodingWorkspace/runs/`\n\n"
...
def render_validation_report(workspace: dict[str, Any]) -> str:
    return (
        "# Validation Report\n\n"
        "## Checks\n"
        f"- Target exists: pass (`{workspace['path']}`)\n"
        "- Model calls: pass (0)\n"
        "- Source edits: pass (none performed by scanner)\n"
        "- AgentLab artifacts: validated by artifact contract after report generation\n\n"
        "## Recommendation\n"
        "READY_FOR_AUDIT\n"
    )


def render_audit_report(target: Path) -> str:
    return (
        "# Audit Report\n\n"
        "## Scope Compliance\n"
        f"- Read-only target: `{target}`\n"
        "- Source edits under target: none\n"
        "- Credential file contents: not read\n\n"
        "## Final Decision\n"
        "READY_FOR_ARCHIVIST\n"
    )


def render_verification_report(workspace: dict[str, Any]) -> str:
    return (
        "# Verification Report\n\n"
        "## Result\n"
        "PASS\n\n"
        f"Workspace memory contains {workspace['top_level_project_count']} top-level projects and "
        f"{workspace['git_repo_count']} git repositories. Artifact contract is checked separately in `artifact_manifest.yml`.\n"
    )


def render_archive_update(project: str, task_id: str, target: Path) -> str:
    return (
        "# Archive Update\n\n"
        "## Task Completed\n"
        f"- Project: `{project}`\n"
